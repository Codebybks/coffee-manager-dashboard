  ...
  async (c: Omit<Customer, 'id' | 'interactions'>) => {
   ...
   const { data, error } = await supabase
    ...
    const newCustomer: Customer = {
      ...
      email:                  data.email,
      phone:                  data.phone,
      preferredOrigin:        data.preferred_origin,
      status:                 data.status,
      assignedSalesRep:       data.assigned_sales_rep,
      nextFollowUpDate:       data.next_follow_up_date,
      notes:                  data.notes,
      certificationsRequired: data.certifications_required
        ? data.certifications_required.split(',').map(s => s.trim())
        : [],
      interactions: []
    };
      ...
      prev.map(c =>
        c.id === customerId
          ? {
              ...
              interactions: [
                ...c.interactions,
                { ...interactionData, id: generateId('int') }
              ]
            }
          : c
      )
